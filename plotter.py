...
class Plotter:
    ...
    def plot(self, legend_labels: list, w=12, h=4):
        ...
        bars = axis.barh(process_names, process_times, left=relative_cumulative_sum, color=colors, edgecolor="black")
        ...
        plt.xticks(np.arange(0, max(relative_cumulative_sum) + max(process_times) + 1, step=1))
        plt.yticks([])
        ...
        plt.text(
            0.5,
            -0.2,
            f"average waiting time: {round(
            self.ave_waiting_time, 2)}",
            ha="center",
            va="center",
    ...
    def plot2(self, legend_labels: list, w=12, h=6):
        ...
        plt.text(
            0.5,
            -0.2,
            f"average waiting time: {round(self.ave_waiting_time, 2)}",
            ha="center",
            va="center",
            transform=axis.transAxes,
        )
